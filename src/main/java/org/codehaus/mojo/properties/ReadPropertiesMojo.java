...
public class ReadPropertiesMojo
    ...
    private void resolveAllFile() throws IOException {
        int length = files.length;
        Map<String, Object> yamlMap = new HashMap<String, Object>();
        Properties properties = new Properties();
        File file = new File(project.getBasedir().getAbsolutePath()+ "/../temp.properties");
        for (int i = 0; i < length; i++) {
            FileReader fileReader = new FileReader(files[i]);
            try {
                if (files[i].getName().endsWith(".yml")) {
                    Map<String, Object> yamlMapObj = yaml.load(fileReader);
                    yamlMap.putAll(resovleMap("", yamlMapObj));
                } else {
     ...
     * @return
     */
    private Map<String, Object> resovleMap(String key, Map map) {
        ...
        for (Object o : map.entrySet()) {
            ...
            if (value instanceof Map) {
                Map<String, Object> stringObjectMap = resovleMap(key + "" + entry.getKey(), (Map) value);
                for (Map.Entry<String, Object> stringObjectEntry : stringObjectMap.entrySet()) {
                    result.put(stringObjectEntry.getKey(), stringObjectEntry.getValue());
                }

            } else {
                result.put(key + "" + entry.getKey(), value);
            }
        }
        return result;
    }
